...
| Date | Change |
|------|--------|
| 2026-07-14 | **Auto Order — DE anchors from DDL comments.** New button in the Field Map's Data Element column header (shown when the parse spec uses `bitmap-fields`). Scans the RAW text of every bound file (comments intact, scoped to the bound DEF): the comment block preceding each field declaration is searched for the last `Bit map position = NN` literal (tolerates `postion`/`pos`, `:` or `=`, any case, `*`-line and inline `!…!` comments) and every DE-capable row whose name matches uniquely gets that value as a `de_map` anchor — anchored pills (accent + ↺) are the user's verification surface; Save commits, Cancel discards. Hand-set anchors are overwritten by comment values. Fields without a comment match keep extrapolating from the nearest anchor; duplicate names are skipped; a ⚠ flash lists fields whose own trailing number disagrees with the commented DE. |
